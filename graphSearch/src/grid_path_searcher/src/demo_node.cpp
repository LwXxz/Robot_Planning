...
{     
    ...
    pathFinding(_start_pt, target_pt);  // find the path, from start_pt to target_pt
}
...
{   
    ...
    pcl::fromROSMsg(pointcloud_map, cloud);  // data transform, from sensor_msgs::PointCloud2 to pcl::PointCloud<pcl::PointXYZ> 
    ...
    pcl::toROSMsg(cloud_vis, map_vis); // data transform, from pcl::PointCloud<pcl::PointXYZ> to sensor_msgs::PointCloud2

    // importance:  publish function uses in subscriber callback function
    map_vis.header.frame_id = "/world";  // dtype: sensor_msgs::PointCloud2 as same as the "_grid_map_vis_pub" publisher
...
int main(int argc, char** argv)
{
    ...
    _map_sub  = nh.subscribe( "map",       1, rcvPointCloudCallBack );  // show the map
    _pts_sub  = nh.subscribe( "waypoints", 1, rcvWaypointsCallback );   // show the path
...
void visGridPath( vector<Vector3d> nodes, bool is_use_jps )
{   
    // use $ rosmsg show [visualization_msgs/Maker] command to show the describe
