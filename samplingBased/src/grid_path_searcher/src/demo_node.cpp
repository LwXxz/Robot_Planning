...
void pathFinding(const Vector3d start_pt, const Vector3d target_pt)
{
    ...
    // Construct a space information instance for this state space, si: space information
    ob::SpaceInformationPtr si(new ob::SpaceInformation(space));
    // Set the object used to check which states in the space are valid
    si->setStateValidityChecker(ob::StateValidityCheckerPtr(new ValidityChecker(si)));
    si->setup();

    // define the scope of the state 
    ...
    if (solved)
    {
        ...
        {
            // in as have the static_cast
            const ob::RealVectorStateSpace::StateType *state = path->getState(path_idx)->as<ob::RealVectorStateSpace::StateType>(); 
            // add the coord to the vector in vector3d style
            path_points.push_back({state->values[0], state->values[1], state->values[2]});
        }
