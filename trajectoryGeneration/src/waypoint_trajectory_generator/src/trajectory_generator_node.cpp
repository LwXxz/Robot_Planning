...
OsqpEigen::Solver solver; // 需要全生命周期存在
...
// for planning
int _poly_num1D;
MatrixXd _polyCoeff;                    // polynomial coefficient
VectorXd _polyTime;                     // polynomial time
Vector3d _startPos = Vector3d::Zero();  // start position
Vector3d _startVel = Vector3d::Zero();  // start velocity
...
{   
    ...
    waypoints.row(0) = _startPos;  // start position always [0, 0, 0]
...
void trajGeneration(Eigen::MatrixXd path)
{
    ...
    visWayPointPath(path);  // path is a matrix(waypoint_list.size() + 1, 3)
...
// TODO part, return the time in the end of each waypoint
VectorXd timeAllocation( MatrixXd Path)
{ 
    VectorXd time(Path.rows() - 1); // time dimension is the vector
    double acc_maxTime = _Vel / _Acc; // from 0 to Vel
    double s_acc = acc_maxTime * _Vel; // include the deceleration part(without /2)
    for (int i = 1; i < Path.rows(); i++){
        double s = (Path.row(i) - Path.row(i-1)).norm(); 
        double cost_time;

        if ( s < s_acc ){
            cost_time = sqrt(2 * s / _Acc);
            time(i) = cost_time;
        } else {    // (s >= s_acc)
            cost_time = acc_maxTime + (s - s_acc) / _Vel; 
            time(i) = cost_time;
        }
    }
    return time;
