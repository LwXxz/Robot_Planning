...
{
// 对SO3数据进行重新编码，尺度统一(归一化)
// 返回值放在函数形参变量中，可以节省一块临时内存变量
                      ...
                      std::vector<uint8_t> &output)
{
  ...
  output.resize(sizeof(so3_cmd_input));                      // allocate space
  memcpy(&output[0], &so3_cmd_input, sizeof(so3_cmd_input)); // copy everything by byte
}
